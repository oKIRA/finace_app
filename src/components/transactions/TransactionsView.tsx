...
export const TransactionsView: React.FC = () => {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-150">
      {/* Top Action Bar */}
      <div className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200 shadow-2xs flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        <div>
        ...
        <div className="flex items-center gap-2 sm:gap-2.5 flex-wrap">
          <button
            ...
            className="px-3 py-2 bg-slate-100 hover:bg-slate-200/80 text-slate-700 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all"
          >
          ...
          <button
            ...
            className="px-3.5 sm:px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-xs transition-all"
          >
      ...
      {/* Transactions List: Table on Desktop, Mobile Cards on Small Devices */}
      <div className="bg-white rounded-2xl border border-slate-200 shadow-2xs overflow-hidden">
        {filteredTransactions.length === 0 ? (
          <div className="p-8 sm:p-12 text-center flex flex-col items-center justify-center">
            ...
            <h4 className="text-sm font-bold text-slate-800">Nenhuma transação encontrada</h4>
            <p className="text-xs text-slate-400 max-w-sm mt-1">
              Altere os filtros de pesquisa ou cadastre uma nova transação.
            </p>
            <button
              onClick={() => openTransactionModal('expense')}
              id="btn-empty-table-add"
              className="mt-4 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-xs"
            >
          ...
          <>
            {/* 1. Mobile & Tablet Card List (< md breakpoint) */}
            <div className="md:hidden divide-y divide-slate-100">
              {filteredTransactions.map((t) => {
                ...
                return (
                  <div key={t.id} className="p-3.5 flex items-center justify-between gap-3 min-w-0">
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5">
                          ...
                          {t.isInstallment && (
                            <span className="px-1 py-0.2 rounded bg-purple-100 text-purple-700 text-[9px] font-black shrink-0">
            ...
            {/* 2. Desktop Full Table (>= md breakpoint) */}
                      ...
                      <tr key={t.id} className="hover:bg-slate-50/80 transition-colors group">
                        ...
                        <td className="py-3 px-4 font-bold text-slate-800">
                          <div className="flex items-center gap-2">
                            ...
                            {t.isInstallment && (
                              <span className="px-1.5 py-0.5 rounded bg-purple-100 text-purple-700 text-[10px] font-black border border-purple-200 shrink-0">
                                {t.currentInstallment}/{t.totalInstallments}
                              </span>
                            )}
                          </div>
                          {t.notes && (
                            <p className="text-[11px] text-slate-400 font-normal mt-0.5 truncate max-w-xs">{t.notes}</p>
                          )}
                        </td>
                        ...
                        <td className="py-3 px-4">
                          <span className="px-2 py-0.5 rounded-md bg-slate-100 text-slate-600 font-semibold text-[10px] uppercase whitespace-nowrap">
